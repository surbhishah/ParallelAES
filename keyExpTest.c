#include<stdio.h>
#define TRUE 1
#define FALSE 0
#define TESTPASS 1
#define TESTFAIL 0
#include"keyExp.c"
int passDetail=FALSE;
int failDetail=FALSE;
/**********************************************************/
int isActualValueExpected(unsigned char actualResult[], unsigned char expectedResult[], int size ){
	int i;
	for(i=0;i<size;i++){
		if(actualResult[i] != expectedResult[i] ){
			return FALSE;
		}
	}
	return TRUE;
}
void printDetail(unsigned char key[], unsigned char expandedKey[], unsigned char expectedExpandedKey[],int sizeKey, int sizeExpandedKey ){
		int i;
		printf("\nKey= ");
		for(i=0 ; i < sizeKey ; i++){
			printf(" %2x", key[i]);
		}
		
		printf("\nActual Expanded Key= ");
		for(i=0 ; i < sizeExpandedKey ; i++){
			if(i%16 ==0)printf("\n");
			printf(" %2x", expandedKey[i]);
		}

		printf("\nExpected Expanded Key= ");
		for(i=0 ; i < sizeExpandedKey ; i++){
			if(i%16 ==0)printf("\n");
			printf(" %2x", expectedExpandedKey[i]);
		}
	
	
}
/**********************************************************/
int testKeyExpansion(unsigned char key[], unsigned char expectedExpandedKey[], int sizeKey, void (*keyExpFuncPtr)()){
	int rounds = sizeKey/4 +6;// rounds = number of rounds 
	const int sizeExpandedKey = 16*(rounds+1);// expected size of expandedKey
	unsigned char expandedKey[sizeExpandedKey];
	int i;
	keyExpFuncPtr(key, expandedKey, sizeKey);
	if(isActualValueExpected(expandedKey, expectedExpandedKey, sizeExpandedKey)){
		if(passDetail == TRUE){
			printDetail(key, expandedKey, expectedExpandedKey, sizeKey, sizeExpandedKey);
		}
		return TESTPASS;
	}
	else{
		if(failDetail == TRUE){
			printDetail(key, expandedKey, expectedExpandedKey, sizeKey, sizeExpandedKey);
		}
		return TESTFAIL;
	}
}
/**********************************************************/
void performTests128(void (*keyExpFuncPtr)()){
	unsigned char key[4][16]={
									{0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00},
								 	{0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff,0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff},
								 	{0x00, 0x01, 0x02, 0x03, 0x04, 0x05, 0x06, 0x07,0x08, 0x09, 0x0a, 0x0b, 0x0c, 0x0d, 0x0e, 0x0f},
								 	{0x69, 0x20, 0xe2, 0x99, 0xa5, 0x20, 0x2a, 0x6d,0x65, 0x6e, 0x63, 0x68, 0x69, 0x74, 0x6f, 0x2a}
								};
	unsigned char expectedExpandedKey[4][176]={
								{
									 
								},
								{
									
									
								},
								{
									,
							 	},
								{
									
								}
							};

	int i;
	unsigned char actualExpandedKey[4][176];
	printf("\n128 bit key tests....");
	for(i=0;i<4;i++){
		if(testKeyExpansion(key[i], expectedExpandedKey[i], 16, keyExpFuncPtr)){
			printf("\nTest Status: Pass");
		}
		else{
			printf("\nTest Status: Fail");			
		}
		printf("\n================================================================");
	}
	
}
/**********************************************************/
void performTests192(void (*keyExpFuncPtr)()){
	unsigned char key[3][24]={
									{0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00},
								 	{0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff,0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff},
								 	{0x00, 0x01, 0x02, 0x03, 0x04, 0x05, 0x06, 0x07,0x08, 0x09, 0x0a, 0x0b, 0x0c, 0x0d, 0x0e, 0x0f, 0x10, 0x11, 0x12, 0x13, 0x14, 0x15, 0x16, 0x17},
								};
	unsigned char expectedExpandedKey[3][208]={
								{
									
								},
								{
									, 	
								},
								{
									
							 	},
							};

	int i;
	unsigned char actualExpandedKey[3][208];
	printf("\n192 bit key tests....");
	for(i=0;i<3;i++){
		if(testKeyExpansion(key[i], expectedExpandedKey[i],24, keyExpFuncPtr)){
			printf("\nTest Status: Pass");
		}
		else{
			printf("\nTest Status: Fail");			
		}
		printf("\n================================================================");
	}
	
}
/**********************************************************/
void performTests256(void (*keyExpFuncPtr)()){
	unsigned char key[3][32]={
									{0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00},
								 	{0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff,0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff},
								 	{0x00, 0x01, 0x02, 0x03, 0x04, 0x05, 0x06, 0x07,0x08, 0x09, 0x0a, 0x0b, 0x0c, 0x0d, 0x0e, 0x0f, 0x10, 0x11, 0x12, 0x13, 0x14, 0x15, 0x16, 0x17, 0x18, 0x19, 0x1a, 0x1b, 0x1c, 0x1d, 0x1e, 0x1f}
								};
	unsigned char expectedExpandedKey[3][240]={
								{
									,
								},
								{
									
								},
								{
									
							 	},
							};

	int i;
	unsigned char actualExpandedKey[3][240];
	printf("\n256 bit key tests....");
	for(i=0;i<3;i++){
		if(testKeyExpansion(key[i], expectedExpandedKey[i],32, keyExpFuncPtr)){
			printf("\nTest Status: Pass");
		}
		else{
			printf("\nTest Status: Fail");			
		}
		printf("\n================================================================");
	}
}
/**********************************************************/
int performTests(void (*keyExpFuncPtr)()){
	performTests128(keyExpFuncPtr);
	performTests192(keyExpFuncPtr);
	performTests256(keyExpFuncPtr);
}
/**********************************************************/
int main(int argc, char** argv){
	if(argc==1){
		passDetail=FALSE;
		failDetail=FALSE;

	}else if(argc >=1){
		if(strcmp(argv[1],"p")==0){
			passDetail=TRUE;
		}
		if(strcmp(argv[1],"f")==0){
			failDetail=TRUE;
		}
		if(strcmp(argv[1],"a")==0){
			passDetail=TRUE;
			failDetail=TRUE;
		}

	}else{
		printf("\nInvalid command line arguments");
		exit(-1);
	}
	void (*keyExpFuncPtr)(unsigned char key[], unsigned char expandedKey[], const int n); 
	keyExpFuncPtr = &keyExpansion;
	performTests(keyExpFuncPtr);
}
/**********************************************************/
